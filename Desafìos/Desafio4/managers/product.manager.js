...
export default class ProductManager{
    ...
    addProduct = async (product) => {
        try {
            ...
            if (typeof product.status !== 'boolean') {
                throw new Error("The 'status' field should be a boolean value (true or false).");
            }
    ...
    try {
        ...
        if (productIndex != -1) {
            if (allProducts.some(p => p.code === product.code && p.id !== id)) {
                throw new Error("You cannot update a product code with an existing one");
            }

            if (typeof product.status !== 'boolean') {
                throw new Error("Status is a required field and should be a boolean value (true or false).");
            }
